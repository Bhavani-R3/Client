...
function Header() {
  const { isLogin, isAdmin, isUser, currentUser, setIsLogin, setIsUser, setIsAdmin, setToken, setCurrentUser } = useContext(AuthContext)

  // logout
  const logoutHandler = async () => {
    if(window.confirm(`Are you sure to logout?`)) {
      await axios.get(`https://rest-api-0f5q.onrender.com/api/auth/logout`)
        .then(res => {
              ...
              <li className='nav-item dropdown'>
              {
                currentUser ? (
                  <NavLink to={`/`} className="nav-link dropdown-toggle" data-bs-toggle="dropdown">
                    <strong className='text-uppercase'> { currentUser ? currentUser.name : "Guest" } </strong> <i className='bi bi-person'></i>
                  </NavLink>
                ) : (
